...
Ціну кожного транспортного засобу можна показувати у **будь-якій валюті**:

- у формі автобуса є поле **«Валюта ціни»** — список із 22 валют (€, ₴, $, zł, Kč, £, ₺, L, lei, Ft, kr, ₾, ₸, AED, ₪, C$ …);
- варіант **«Інша»** + поле **«Своя валюта»** дозволяє вписати будь-який символ чи код (`грн`, `₴`, `AED`, `руб`);
- якщо у транспорту валюта не вибрана — використовується **валюта за замовчуванням** із розділу «Налаштування» (типово **₴ гривня**);
- одиниця розрахунку («за годину / за день / за поїздку / за кілометр») вказується **без валюти** — валюту визначає окреме поле;
- зміна валюти в адмінці одразу змінює ціну на сайті (і в картці, і в галереї).

---

## 3. Telegram-бот (заявки + адміністратори)
...
## 5. Безкоштовна зовнішня база даних (не на вашому сервері)
...
## 7. Публікація на GitHub і деплой на Vercel (безкоштовно)
...
## 9. Продуктивність і стійкість (важливо для зовнішньої БД)
...
npm run test:mobile             # 23 перевірки адаптації: 320/360/390/430px + адмінка з телефона
```

Наскрізні перевірки (`tests/e2e.mjs`, 75 сценаріїв) проходять і на локальній базі,
